...
void init_matrix(int32_t *matrix, const int32_t *src,
...
int verify_matrix(int32_t *matrix, const int32_t *checksum,
  ...
  for (unsigned int i = 0; i < num_rows; ++i) {
    ...
    int32_t diff = sum - *((int32_t *)checksum + i);
    if (diff < 0)
      diff = -diff;
    if (diff > 0.001) {
      return i == 0 ? -1 : (int)i;
...
int main() {
  ...
  const int32_t *a = gemm_A_dram;
  const int32_t *b = gemm_B_dram;
  int32_t *c = gemm_C_dram;
  ...
  float utilization = 100 * performance / (2.0 * 2 * NR_LANES);
  ...
  int error = verify_matrix(c, (const int32_t *)gemm_checksum, M, N);
  if (error != 0) {
    printf("Error code %d\n", error);
    printf("c[%d]=%d\n", error, c[error]);
    return error;
