...
void matmul(int64_t *c, const int64_t *a, const int64_t *b, int64_t M, int64_t N, int64_t P) {
  ...
  asm volatile ("vsetvli %0, %1, e64, m4" : "=r" (block_size_n) : "r" (N));

  // Slice the matrix into a manageable number of columns n_
  for (int64_t n = 0; n < N; n += block_size_n) {
    // Set the vector length
    int64_t n_ = MIN(N - n, block_size_n);
    asm volatile ("vsetvli t0, %0, e64, m4" :: "r" (n_));

    // Find pointers to the submatrices
    const int64_t *b_ = b + n;
    int64_t *c_ = c + n;

    // Iterate over the rows
    for (int64_t m = 0; m < M; m += block_size) {
      // Find pointer to the submatrices
      const int64_t *a_ = a + m*P;
      int64_t *c__ = c_ + m*N;

      // Call the kernels
      matmul_vec_4x4_slice_init(c_, N);
...
void matmul_vec_4x4(int64_t *c, const int64_t *a, const int64_t *b, int64_t N, int64_t P) {
  ...
  while (p < P) {
    // Load one row of B
    asm volatile ("vle64.v v20, (%0); add %0, %0, %1" : "+r" (b) : "r" (ldb)); p++;

    // Calculate pointer to the matrix A
    a = (const int64_t*) a_ + p;
    ...
    // Load one row of B
    asm volatile ("vle64.v v16, (%0); add %0, %0, %1" : "+r" (b) : "r" (ldb)); p++;
    a = (const int64_t*) a_ + p;
