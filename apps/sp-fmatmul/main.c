...
void init_matrix(float *matrix, const float *src, const unsigned int rows_start,
...
int main() {
  ...
  const float *a = gemm_A_dram;
  const float *b = gemm_B_dram;
  float *c = gemm_C_dram;
  ...
  int error = verify_matrix(c, (const float *)gemm_checksum, M, N);
  if (error != 0) {
    printf("Error code %d\n", error);
    printf("c[%d]=%d\n", error, c[error]);
    return error;
