...
void init_matrix(int16_t *matrix, const int16_t *src,
...
// Verify the matrices
int verify_matrix(int16_t *matrix, const int16_t *checksum,
                  const unsigned int num_rows, const unsigned int num_columns) {
  for (unsigned int i = 0; i < num_rows; ++i) {
    int16_t sum = 0;
    for (unsigned int j = 0; j < num_columns; ++j) {
      sum += *((int16_t *)matrix + i * num_columns + j);
    }

    int16_t diff = sum - *((int16_t *)checksum + i);
    if (diff < 0)
      diff = -diff;
    if (diff > 0.001) {
      return i == 0 ? -1 : (int)i;
...
int main() {
  ...
  const int16_t *a = gemm_A_dram;
  const int16_t *b = gemm_B_dram;
  int16_t *c = gemm_C_dram;
  ...
  int error = verify_matrix(c, (const int16_t *)gemm_checksum, M, N);
  if (error != 0) {
    printf("Error code %d\n", error);
    printf("c[%d]=%d\n", error, c[error]);
    return error;
