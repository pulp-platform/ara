...
/*
  ...
  b) Calculate its contributions of 2 output channels the F = 3 output rows
  using one column of the filter c) Slide down the input row by 1, injecting the
  next input scalar element in the tail d) Repeat from b), taking the next colum
  of the filter, until the last column is fetched d2) Repeat from a) for all the
  channels e) Store the first output row, the one that is complete f) Move all
  the output rows up by one register, to restore the initial condition g) Repeat
  from a)
...
void fconv3d_CHx3x3_block(double *o, double *i, double *f, int64_t M, int64_t N,
  ...
  for (int och = 0; och < OC / 2; ++och) {
    ...
    for (int ch = 0; ch < C; ++ch) {
      ...
      asm volatile(
          "vfslide1down.vf v2, v0, %0" ::"f"(*i_slide_ptr_0++)); // SLIDE
      ...
      asm volatile(
          "vfslide1down.vf v6, v4, %0" ::"f"(*i_slide_ptr_1++)); // SLIDE
      ...
      asm volatile(
          "vfslide1down.vf v10, v8, %0" ::"f"(*i_slide_ptr_2++)); // SLIDE
      ...
      asm volatile(
          "vfslide1down.vf v4, v6, %0" ::"f"(*i_slide_ptr_1++)); // SLIDE
      ...
      asm volatile(
          "vfslide1down.vf v8, v10, %0" ::"f"(*i_slide_ptr_2++)); // SLIDE
      ...
      asm volatile(
          "vfslide1down.vf v12, v14, %0" ::"f"(*i_slide_ptr_3++)); // SLIDE
    ...
    for (int row = F - 1; row < M - (F - 1); row++) {
      ...
      for (int ch = 0; ch < C; ch++) {
        ...
        int64_t base_idx_0 = 2 * och * fch_full_len + ch * fch_len;
        int64_t base_idx_1 =
            2 * och * fch_full_len + fch_full_len + ch * fch_len;
      ...
      asm volatile("vse64.v v28, (%0); add %0, %0, %1"
                   : "+&r"(o_p1)
                   : "r"(ldo));
    ...
    asm volatile("vse64.v v20, (%0); add %0, %0, %1" : "+&r"(o) : "r"(ldo));
    asm volatile("vse64.v v22, (%0); add %0, %0, %1"
                 : "+&r"(o)
                 : "r"(ldo + ((M * N) << 3)));
