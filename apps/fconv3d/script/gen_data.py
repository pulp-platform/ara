...
for ch in range(CH):
        result += np.around(scipy.signal.convolve2d(np.flip(gen_filter[ch]), image[ch], 'valid')).astype(np.float64) # https://stackoverflow.com/questions/41613155/what-does-scipy-signal-convolve2d-calculate
