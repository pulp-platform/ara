...
void fconv3d_CHx7x7(int16_t *o, int16_t *i, int16_t *f, int64_t M, int64_t N,
                    int64_t C, int64_t F) {
...
void fconv3d_CHx7x7_warm(int16_t *o, int16_t *i, int16_t *f, int64_t M, int64_t N,
                    int64_t C, int64_t F) {
...
void fconv3d_CHx7x7_block(int16_t *o, int16_t *i, int16_t *f, int64_t M, int64_t N,
                          int64_t n_, int64_t C, int64_t F) {

  // Helper variables
  int64_t ldo = N << 1;
  int64_t ldi_pad = (N + F - 1) << 1;
  ...
  int16_t fl0, fl1, fl2, fl3, fl4, fl5, fl6;
  // Buffers for coefficients preloading (solve 16-lane starvation problem)
  int16_t f0_buf, f1_buf, f2_buf, f3_buf, f4_buf, f5_buf, f6_buf;

  int16_t *i_slide_ptr_0;
  int16_t *i_slide_ptr_1;
  int16_t *i_slide_ptr_2;
  int16_t *i_slide_ptr_3;
  ...
  for (int ch = 0; ch < C; ++ch) {
    ...
    asm volatile("vle16.v v0, (%0); add %0, %0, %1"
                 : "+&r"(i__)
                 : "r"(ldi_pad));
    asm volatile("vle16.v v4, (%0); add %0, %0, %1"
                 : "+&r"(i__)
                 : "r"(ldi_pad));
    asm volatile("vle16.v v8, (%0); add %0, %0, %1"
                 : "+&r"(i__)
                 : "r"(ldi_pad));
    asm volatile("vle16.v v12, (%0); add %0, %0, %1"
                 : "+&r"(i__)
                 : "r"(ldi_pad));
  ...
  for (int ch = 0; ch < C; ++ch) {
    ...
    asm volatile("vle16.v v2, (%0); add %0, %0, %1"
                 : "+&r"(i__)
                 : "r"(ldi_pad));
    asm volatile("vle16.v v6, (%0); add %0, %0, %1"
                 : "+&r"(i__)
                 : "r"(ldi_pad));
    asm volatile("vle16.v v10, (%0); add %0, %0, %1"
                 : "+&r"(i__)
                 : "r"(ldi_pad));
  ...
  asm volatile("vse16.v v16, (%0); add %0, %0, %1" : "+&r"(o) : "r"(ldo));
  ...
  asm volatile("vse16.v  v18, (%0); add %0, %0, %1" : "+&r"(o) : "r"(ldo));
  ...
  asm volatile("vse16.v  v20, (%0); add %0, %0, %1" : "+&r"(o) : "r"(ldo));
  ...
  asm volatile("vse16.v  v22, (%0); add %0, %0, %1" : "+&r"(o) : "r"(ldo));
...
void fconv3d_warm(int16_t *o, int16_t *i, int16_t *f, int64_t M, int64_t N,
                          int64_t n_, int64_t C, int64_t F) {
...
/*
  ...
  for (int k = 0; k < F; ++k) {
    ...
    asm volatile("vmacc.vx v16, %0, v0" :: "f"(f[42  + (2*k)]));
    asm volatile("vmacc.vx v18, %0, v0" :: "f"(f[35  + (2*k)]));
    asm volatile("vmacc.vx v20, %0, v0" :: "f"(f[28 + (2*k)]));
    asm volatile("vmacc.vx v22, %0, v0" :: "f"(f[21 + (2*k)]));
    asm volatile("vmacc.vx v24, %0, v0" :: "f"(f[14 + (2*k)]));
    asm volatile("vmacc.vx v26, %0, v0" :: "f"(f[7 + (2*k)]));
    if (k == 0)
      asm volatile("vmul.vx v28, v0, %0" :: "f"(f[0 + (2*k)]));
    else
      asm volatile("vmacc.vx v28, %0, v0" :: "f"(f[0 + (2*k)]));

    // Slide the input row by one, and inject the next scalar element of the row
    asm volatile("vslide1down.vx v0, v0, %0" :: "f"(*i_slide_ptr_0++));
  }
