...
// Compiler support: https://github.com/riscv/riscv-gnu-toolchain/tree/rvv-intrinsic
#ifdef INTRINSICS
...
// Scalar dropout
void dropout_gold(const unsigned int n, const float *i, const float scale, const int32_t *sel, float *o) {
...
#ifdef INTRINSICS
  void dropout_vec(const unsigned int n, const float *i, const float scale, const int32_t *sel, float *o) {
    unsigned int vl;
    ...
    for (unsigned int avl = n; (vl = vsetvl_e32m8(avl)) > 0; avl -= vl) {
      ...
      vo = vfmul_vf_f32m8_m(vsel_m, vo, vi, scale);
      vse32_v_f32m8 (o, vo);
      // Bump pointers
      i   += vl;
      sel += vl;
  ...
  void dropout_vec(const unsigned int n, const float *i, const float scale, const int32_t *sel, float *o) {
    ...
    asm volatile ("vsetvli %[vl], %[n], e32, m8" : [vl] "=r" (vl) : [n] "r" (n));

    for (unsigned int avl = n; avl > 0; avl -= vl) {
      // Find next vl
      asm volatile ("vsetvli %[vl], %[avl], e32, m8" : [vl] "=r" (vl) : [avl] "r" (avl));
      // Load input and selection vectors
