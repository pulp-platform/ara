...
#endif
#else
    #error "Please define DTYPE. Example command: make bin/dtype-matmul ENV_DEFINES='-DDTYPE=FLOAT64' def_args_dtype-matmul='float64 128 128 128'"
#endif
...
int main() {
  ...
  int error = _VERIFY(c, (const double *)g, M, P, THRESHOLD);
  if (error != 0) {
