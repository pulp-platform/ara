...
#include "dp-imatmul.h"

// Verify the matrix
int dp_imatmul_verify(int64_t *result, int64_t *gold, size_t R, size_t C) {
...
void dp_imatmul(int64_t *c, const int64_t *a, const int64_t *b,
                const unsigned int M, const unsigned int N,
                const unsigned int P) {
  if (M <= 4) {
    dp_imatmul_4x4(c, a, b, M, N, P);
  } else if (M <= 8) {
    dp_imatmul_8x8(c, a, b, M, N, P);
  } else if (M <= 64) {
    dp_imatmul_16x16(c, a, b, M, N, P);
  } else if (M <= 128) {
    // Vector length is 64 elements. With an 8x8 dp_imatmul,
    // we can use LMUL=2, having a vl of 128.
    dp_imatmul_8x8(c, a, b, M, N, P);
  } else {
    // Vector length is 64 elements. With an 4x4 dp_imatmul,
...
void dp_imatmul_4x4(int64_t *c, const int64_t *a, const int64_t *b,
                    const unsigned int M, const unsigned int N,
                    const unsigned int P) {
  ...
  for (unsigned int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned int m = 0; m < M; m += block_size) {
      ...
      dp_imatmul_vec_4x4_slice_init();
      dp_imatmul_vec_4x4(c__, a_, b_, N, P);
    }
  }
}

void dp_imatmul_vec_4x4_slice_init() {
...
void dp_imatmul_vec_4x4(int64_t *c, const int64_t *a, const int64_t *b,
...
void dp_imatmul_8x8(int64_t *c, const int64_t *a, const int64_t *b,
                    const unsigned int M, const unsigned int N,
                    const unsigned int P) {
  // We work on 4 rows of the matrix at once
  ...
  for (unsigned int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned int m = 0; m < M; m += block_size) {
      ...
      dp_imatmul_vec_8x8_slice_init();
      dp_imatmul_vec_8x8(c__, a_, b_, N, P);
    }
  }
}

void dp_imatmul_vec_8x8_slice_init() {
  asm volatile("vmv.v.i v0,  0");
...
void dp_imatmul_vec_8x8(int64_t *c, const int64_t *a, const int64_t *b,
                        const unsigned int N, const unsigned int P) {
  // Temporary variables
...
void dp_imatmul_16x16(int64_t *c, const int64_t *a, const int64_t *b,
                      const unsigned int M, const unsigned int N,
                      const unsigned int P) {
  // We work on 4 rows of the matrix at once
  ...
  for (unsigned int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned int m = 0; m < M; m += block_size) {
      ...
      dp_imatmul_vec_16x16_slice_init();
      dp_imatmul_vec_16x16(c__, a_, b_, N, P);
    }
  }
}

void dp_imatmul_vec_16x16_slice_init() {
  asm volatile("vmv.v.i v0,  0");
...
void dp_imatmul_vec_16x16(int64_t *c, const int64_t *a, const int64_t *b,
                          const unsigned int N, const unsigned int P) {
  // Temporary variables
  int64_t t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15;
  ...
  asm volatile("ld %[t], (%[a])" : [t] "=r"(t7) : [a] "r"(a));
  a += N;
  asm volatile("ld %[t], (%[a])" : [t] "=r"(t8) : [a] "r"(a));
  a += N;
  asm volatile("ld %[t], (%[a])" : [t] "=r"(t9) : [a] "r"(a));
  a += N;
  asm volatile("ld %[t], (%[a])" : [t] "=r"(t10) : [a] "r"(a));
  a += N;
  asm volatile("ld %[t], (%[a])" : [t] "=r"(t11) : [a] "r"(a));
  a += N;
  asm volatile("ld %[t], (%[a])" : [t] "=r"(t12) : [a] "r"(a));
  a += N;
  asm volatile("ld %[t], (%[a])" : [t] "=r"(t13) : [a] "r"(a));
  a += N;
  asm volatile("ld %[t], (%[a])" : [t] "=r"(t14) : [a] "r"(a));
  a += N;
  asm volatile("ld %[t], (%[a])" : [t] "=r"(t15) : [a] "r"(a));
