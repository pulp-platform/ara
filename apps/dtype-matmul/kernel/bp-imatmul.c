...
void bp_imatmul(int8_t *c, const int8_t *a, const int8_t *b,
  ...
  if (M <= 4) {
    bp_imatmul_4x4(c, a, b, M, N, P);
  } else if (M <= 8) {
    bp_imatmul_8x8(c, a, b, M, N, P);
  } else if (M <= 64) {
    bp_imatmul_16x16(c, a, b, M, N, P);
  } else if (M <= 128) {
    // Vector length is 64 elements. With an 8x8 bp_imatmul,
    // we can use LMUL=2, having a vl of 128.
    bp_imatmul_8x8(c, a, b, M, N, P);
  } else {
    // Vector length is 64 elements. With an 4x4 bp_imatmul,
...
void bp_imatmul_4x4(int8_t *c, const int8_t *a, const int8_t *b,
  ...
  for (unsigned int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned int m = 0; m < M; m += block_size) {
      ...
      bp_imatmul_vec_4x4_slice_init();
      bp_imatmul_vec_4x4(c__, a_, b_, N, P);
    }
  }
}
...
void bp_imatmul_8x8(int8_t *c, const int8_t *a, const int8_t *b,
  ...
  for (unsigned int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned int m = 0; m < M; m += block_size) {
      ...
      bp_imatmul_vec_8x8_slice_init();
      bp_imatmul_vec_8x8(c__, a_, b_, N, P);
    }
  }
}
...
void bp_imatmul_16x16(int8_t *c, const int8_t *a, const int8_t *b,
  ...
  for (unsigned int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned int m = 0; m < M; m += block_size) {
      ...
      bp_imatmul_vec_16x16_slice_init();
      bp_imatmul_vec_16x16(c__, a_, b_, N, P);
    }
  }
}
...
void bp_imatmul_vec_16x16(int8_t *c, const int8_t *a, const int8_t *b,
                          const unsigned int N, const unsigned int P) {
  // Temporary variables
  int8_t t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15;
  ...
  asm volatile("lb %[t], (%[a])" : [t] "=r"(t7) : [a] "r"(a));
  a += N;
  asm volatile("lb %[t], (%[a])" : [t] "=r"(t8) : [a] "r"(a));
  a += N;
  asm volatile("lb %[t], (%[a])" : [t] "=r"(t9) : [a] "r"(a));
  a += N;
  asm volatile("lb %[t], (%[a])" : [t] "=r"(t10) : [a] "r"(a));
  a += N;
  asm volatile("lb %[t], (%[a])" : [t] "=r"(t11) : [a] "r"(a));
  a += N;
  asm volatile("lb %[t], (%[a])" : [t] "=r"(t12) : [a] "r"(a));
  a += N;
  asm volatile("lb %[t], (%[a])" : [t] "=r"(t13) : [a] "r"(a));
  a += N;
  asm volatile("lb %[t], (%[a])" : [t] "=r"(t14) : [a] "r"(a));
  a += N;
  asm volatile("lb %[t], (%[a])" : [t] "=r"(t15) : [a] "r"(a));
