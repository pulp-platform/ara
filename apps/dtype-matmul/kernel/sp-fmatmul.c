...
void sp_fmatmul(float *c, const float *a, const float *b, const unsigned int M,
  ...
  if (M <= 4) {
    sp_fmatmul_4x4(c, a, b, M, N, P);
  } else if (M <= 8) {
    sp_fmatmul_8x8(c, a, b, M, N, P);
  } else if (M <= 64) {
    sp_fmatmul_16x16(c, a, b, M, N, P);
  } else if (M <= 128) {
    // Vector length is 64 elements. With an 8x8 sp_fmatmul,
    // we can use LMUL=2, having a vl of 128.
    sp_fmatmul_8x8(c, a, b, M, N, P);
  } else {
    // Vector length is 64 elements. With an 4x4 sp_fmatmul,
...
void sp_fmatmul_4x4(float *c, const float *a, const float *b,
  ...
  for (unsigned int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned int m = 0; m < M; m += block_size) {
      ...
      sp_fmatmul_vec_4x4_slice_init();
      sp_fmatmul_vec_4x4(c__, a_, b_, N, P);
    }
  }
}
...
void sp_fmatmul_vec_4x4(float *c, const float *a, const float *b,
  ...
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t0) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t1) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t2) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t3) : [a] "r"(a));
...
void sp_fmatmul_8x8(float *c, const float *a, const float *b,
                    const unsigned int M, const unsigned int N,
                    const unsigned int P) {
  // We work on 4 rows of the matrix at once
  ...
  for (unsigned int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned int m = 0; m < M; m += block_size) {
      ...
      sp_fmatmul_vec_8x8_slice_init();
      sp_fmatmul_vec_8x8(c__, a_, b_, N, P);
    }
  }
}
...
void sp_fmatmul_vec_8x8(float *c, const float *a, const float *b,
  ...
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t3) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t4) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t5) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t6) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t7) : [a] "r"(a));
...
void sp_fmatmul_16x16(float *c, const float *a, const float *b,
  ...
  for (unsigned int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned int m = 0; m < M; m += block_size) {
      ...
      sp_fmatmul_vec_16x16_slice_init();
      sp_fmatmul_vec_16x16(c__, a_, b_, N, P);
    }
  }
}
...
void sp_fmatmul_vec_16x16(float *c, const float *a, const float *b,
  ...
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t7) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t8) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t9) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t10) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t11) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t12) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t13) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t14) : [a] "r"(a));
  a += N;
  asm volatile("flw %[t], (%[a])" : [t] "=f"(t15) : [a] "r"(a));
