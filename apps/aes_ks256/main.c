...
extern const uint32_t keys_lo[] __attribute__((aligned(4 * NR_LANES)));
extern const uint32_t keys_hi[] __attribute__((aligned(4 * NR_LANES)));
...
int main() {
  ...
  printf("==============\n");
  printf("\n");
  printf("Expanding %lu AES-256 keys (%lu input bytes).\n", (unsigned long)N,
         (unsigned long)(N * AES_256_KEY_BYTES));

  start_timer();
  aes256_expand_keys_vec(keys_lo, keys_hi, round_keys, N);
  stop_timer();

  const int64_t runtime = get_timer();
  const size_t bytes = N * AES_256_KEY_BYTES;
  if (runtime > 0) {
  ...
  for (uint64_t i = 0; i < N * AES_256_NKEYS * AES_BLOCK_WORDS; ++i) {
