...
// *********** NMAra Testing *********** //
  ...
  for (uint64_t avl = vsize; avl <= vsize; avl *= 2) {
    ...
    // if (SCALAR) {
    //   printf("Calulating 64b dotp with scalar with length = %lu\n", avl);
    //   start_timer();
    //   printf("Start Execution.\n");
    //   faxpy_s64b(a, v64x, v64y, avl);
    //   printf("Stop Execution.\n");
    //   stop_timer();
    //   printf("Stop Timer.\n");
    //   runtime_s = get_timer();
    //   printf("Scalar runtime: %ld\n", runtime_s);
    //   printf("64b: Scalar Core Result: %f, Golden Result: %f\n", res64_s, gold64);
    // }

    if (avl == vsize){
      if (CHECK) {
        // if (SCALAR) {
        //   printf("Checking scalar core results...\n");
        //   for (unsigned int i = 0; i < vsize; i++) {
        //     if (fp_check(v64y_scalar[i], gold64[i])) {
        //       PRINTF("Error: Index %d -> Result = %f, Expected = %f\n", i,
        //             (float)v64y_scalar[i], (float)gold64[i]);
        //     }
        //   }
        // }
        printf("Checking vector engine results...\n");
        for (unsigned int i = 0; i < vsize; i++) {
          if (fp_check(v64y[i], gold64[i])) {
            printf("Error: Index %d -> Result = %f, Expected = %f\n", i,
                  (float)v64y[i], (float)gold64[i]);
          }
        }
      }
    }
