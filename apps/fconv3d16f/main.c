...
// Verify the matrices
int verify_matrix(__fp16 *matrix, __fp16 *golden_matrix, int64_t R, int64_t C, float threshold) {
...
void print_matrix(__fp16 const *matrix, uint64_t num_rows,
...
void print_matrix_simple(__fp16 const *matrix, uint64_t num_rows, uint64_t num_columns) {
    for (uint64_t i = 0; i < num_rows; ++i) {
        for (uint64_t j = 0; j < num_columns; ++j) {
...
int main() {
  ...
  int error = verify_matrix(o, golden_o, M, N,   0.5);
  if (error != 0) {
    printf("Fail.\n");
  } else {
    printf("Passed.\n");
