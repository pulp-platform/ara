...
// Verify the matrices
int verify_matrix(int32_t *matrix, int32_t *golden_matrix, int64_t R, int64_t C, double threshold) {
    for (int r = 0; r < R; ++r) {
        for (int c = 0; c < C; ++c) {
            int32_t mat_val = matrix[c + C * r];
            int32_t gold_val = golden_matrix[c + C * r];
            if (mat_val != gold_val) {
