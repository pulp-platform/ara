...
void conv2d_vec_4xC_7x7(int64_t *o, int64_t *i, int64_t *f, int64_t C, int64_t F) {
  ...
  asm volatile ("vsetvli zero, %0, e64, m2" :: "r" (C));
  f_ = f;
  // Fetch the first column of the filter, and start calculating its contribution on the two output rows (v0, v2)
  ...
  for (int64_t idx = 1; idx < F-1; ++idx) {
    ...
    f_ = f + idx;
    slamt = idx;
    // Fetch the other columns of the filter (except for the last one), and start calculating their contributions on the two output rows (v0, v2)
    // To do so, at each iteration slide down the input rows by one
    asm volatile ("ld %1, (%0); add %0, %0, %2" : "+&r" (f_), "=&r" (t0) : "r" (ldf));
