...
void conv2d_3x3(int64_t *o, int64_t *i, int64_t *f, int64_t R, int64_t C, int64_t F) {
  ...
  // The first F-1 rows have already been loaded by conv2d_vec_4xC_slice_preload_3x3()
...
void conv2d_vec_4xC_3x3(int64_t *o, int64_t *i, int64_t *f, int64_t C, int64_t F) {
  ...
  asm volatile ("vsetvli zero, %0, e64, m2" :: "r" (C));
  f_ = f;
  // Fetch the first column of the filter, and start calculating its contribution on the four output rows (v0, v2, v4, v6)
  ...
  // To do so, slide down the input rows by one
  asm volatile ("ld %1, (%0); add %0, %0, %2" : "+&r" (f_), "=&r" (t0) : "r" (ldf));
