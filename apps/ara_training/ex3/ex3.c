...
void ex3(velement_t *i, velement_t *sel, velement_t *o, velement_t *o_gold) {
  ...
  double performance = (double)N / get_timer(); // SEW-WIDE useful OPs/cycle
  printf("The maximum BW-limited Ara performance for SEW = %d would be %lf OP/cycle\n",
      SEW, (double)16 / (3 * (1 << sew_byte_exp)));
  printf("Ara's performance is %lf OP/cycle, or %lf %% of the BW-limited peak performance.\n",
      performance, (double)100*performance * 3 * (1 << sew_byte_exp) / 16);

  // Q1: Did your kernel achieve a high performance?

  // Q2: Analyze the dropout performance finding its arithmetic intensity.
  // How can you improve the performance of the kernel?
