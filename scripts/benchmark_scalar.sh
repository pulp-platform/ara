...
compile_and_run() {
  ...
  config=${config} make -C hardware/ -B $sim app=benchmarks ${id_opt} || exit
}

compile_and_run_scalar() {
  kernel=$1
  defines=$2
  tempfile=$3
  id=$4
  ...
  echo "Compiling ${kernel}${id_suffix} scalar benchmark:"
  config=${config} ENV_DEFINES="-D${kernel^^}=1 -DSCALAR=1 $defines" \
         make -C apps/ bin/benchmarks${id_suffix} || exit
  echo "Simulating ${kernel}${id_suffix}:"
  config=${config} make -C hardware/ -B $sim app=benchmarks ${id_opt} || exit
}
...
matmul() {
  ...
  for size in 4 8 16 32 64 128; do
    ...
    compile_and_run_scalar $kernel "$defines" $tempfile 0 || exit
  done
}
